...
function mapearDatosAlbum(albumData) {
  ...
  let fecha_estreno = albumData.releasedate ? albumData.releasedate.trim() : '';
  if (!fecha_estreno && albumData.wiki && albumData.wiki.published) {
    // Intenta obtener la fecha de wiki.published
    fecha_estreno = albumData.wiki.published.split(',')[0].trim(); 
  } else if (!fecha_estreno && albumData.wiki && albumData.wiki.summary) {
    // A veces la fecha está en el summary
    const yearMatch = albumData.wiki.summary.match(/\b(19|20)\d{2}\b/);
    if (yearMatch) {
      fecha_estreno = yearMatch[0];
    }
  }
...
/**
 * Busca álbumes en la API de Last.fm y devuelve múltiples resultados.
 */
async function searchAlbumsEnLastfm(query) {
  try {
    const API_KEY = process.env.LASTFM_API_KEY;
    const response = await axios.get('http://ws.audioscrobbler.com/2.0/', {
      params: {
        ...
        api_key: API_KEY,
        album: query,
        limit: 12, // Limitamos a 12 resultados
        format: 'json'
      }
    });

    if (!response.data.results || !response.data.results.albummatches) {
      throw new Error('No se encontraron resultados.');
    }

    // Mapear los resultados a un formato más amigable
    return response.data.results.albummatches.album.map(album => ({
      imdbID: album.mbid || album.name, // Usamos imdbID para mantener consistencia con el frontend
      Title: album.name,
      Year: album.releasedate ? album.releasedate.split('-')[0] : 'N/A',
      Poster: album.image.find(img => img.size === 'extralarge')?.['#text'] || '',
      Type: 'album',
      Artist: album.artist,
      // Datos adicionales que podrían ser útiles
      Listeners: album.listeners || 'N/A',
      PlayCount: album.playcount || 'N/A',
      // Datos necesarios para el refresh
      album_id: album.mbid || album.name,
      nombre: album.name,
      artista: {
        artista_id: album.artistmbid || '',
        nombre: album.artist
      },
      fecha_estreno: album.releasedate ? album.releasedate.trim() : '',
      portada: album.image.find(img => img.size === 'extralarge')?.['#text'] || '',
      canciones: [] // Se obtendrán al hacer refresh
    }));
  } catch (error) {
    console.error('Error al buscar álbumes en Last.fm:', error.message);
    throw error;
  }
}

module.exports = { 
  fetchAlbumFromLastfm, 
  saveAlbumFromLastfm,
  searchAlbumsEnLastfm 
};
