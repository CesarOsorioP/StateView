...
async function agregarPeliculaPorId(req, res) {
  try {
    ...
    res.status(500).json({ error: 'Error agregando la película: ' + error.message });
  }
}
...
async function buscarPeliculasEnOMDb(req, res) {
  try {
    ...
    const resultados = await searchPeliculasEnOMDb(query);
    res.json({
      success: true,
      data: resultados
    });
  } catch (error) {
    console.error('Error en buscarPeliculasEnOMDb:', error);
    res.status(500).json({ 
      success: false,
      error: error.message 
    });
